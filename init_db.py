...
def insert_all():
    ...
    data = requests.get(
        'https://search.daum.net/search?w=tot&q=역대관객순위&DA=MOR&rtmaxcoll=MOR', headers=headers)
    ...
    for movie in movies:
        ...
        if not tag_element:
            continue
        open_date = tag_element.string.replace(
            '.', ' ').strip().replace(' ', '.')
        ...
        last_id = db.movies.find_one(sort=[("_id", -1)])
        ...
        print('완료: ', title, open_year, open_month,
              open_day, viewers, poster_url, info_url)
