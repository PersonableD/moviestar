...
def show_movies():
    ...
    if sortMode == 'date':
        movies = list(db.movies.find(
            {'trashed': False}).sort('open_date', -1))
    else:
        jsonify({'result': 'failure'})
...
def like_movie():
    ...
    print(data_id)
    movie = db.movies.find_one({'_id': int(data_id)})
    ...
    result = db.movies.update_one({'_id': int(data_id)}, {
                                  '$set': {'likes': new_likes}})
...
def trashed_movies_post():
    trashMode = request.form['trashMode']
    data_id = request.form['id']
    if trashMode == 'false':
        db.movies.update_one({'_id': int(data_id)}, {
            '$set': {'trashed': True}})
    if trashMode == 'true':
        db.movies.update_one({'_id': int(data_id)}, {
            '$set': {'trashed': False}})
...
@app.route('/api/list/trash', methods=['DELETE'])
def articles_delete():
    data_id = request.form['id']

    result = db.movies.delete_one({'_id': int(data_id)})
